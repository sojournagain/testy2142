...
// testcoin2142: total coins staked (non-spendable until maturity)
        ...
        CTxDB txdb("r");
        {
            ...
            while (true)
            {
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-testcoin2142-address
...
// testcoin2142: get current stake weight
...
bool CWallet::SetAddressBookName(const CTxDestination& address, const string& strName)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).WriteName(Ctestcoin2142Address(address).ToString(), strName);
}
...
bool CWallet::DelAddressBookName(const CTxDestination& address)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).EraseName(Ctestcoin2142Address(address).ToString());
}
...
// testcoin2142: check 'spent' consistency between wallet and txindex
// testcoin2142: fix wallet spent state according to txindex
...
// testcoin2142: disable transaction (only for coinstake)
