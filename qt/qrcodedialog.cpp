...
{
    QString ret = QString("testcoin2142:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non tc2 unit input in lnReqAmount, we generate the URI with tc2 as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(testcoin2142Units::format(testcoin2142Units::tc2, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
